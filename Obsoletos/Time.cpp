...
void CTimeManager::CalculateFPS(void)					// Calculates fps (current)
{
	...
	GameState = Game.AI->GetState();
	switch (GameState)
	{
	case CG_PASSING_2_3D:
	case CG_INTERMISSION:
	case CG_LOST_LIFE:
	case CG_WON:
	case CG_LOST_GAME:
	case CG_FADING_OUT2WON:
	case CG_FADING_OUT2END:
	case CG_FADING_OUT2LOST:
		intermede_ms = fps;
		break;
	}
...
void CTimeManager::CalculateFPS2(void)					// Calculates fps 2
{
