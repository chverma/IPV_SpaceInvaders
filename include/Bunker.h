...
#define CBNK_BRICK_WIDTH	0.1f
#define CBNK_BRICK_HEIGHT	0.1f
...
extern char *CBNK_TypeNames[CBNK_MAXTYPE];
...
class CBunker: public CCharacter
{
	//Atributes
public:
	unsigned int	FileAvailable,			///<Pointer to the current available file. For initialization purpouses
					ColumnAvailable;		///<Pointer to the current available column. For initialization purpouses
	...
	CBunker(){Init();}					///<Constructor of the class
	...
	#ifdef CHAR_USE_AABB
		void UpdateAABB();	///<Recalculates its AABB given a brick
