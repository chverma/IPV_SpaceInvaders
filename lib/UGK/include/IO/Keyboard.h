...
#define CK_ACTIONS std::list<CK_TOKEN> //Where we store the actions 
...
private:
	...
	void SetActiveKeys(const CK_TOKEN);	///<Set a new key into the active list of key pressed
	void ResetActiveTokens(const CK_TOKEN);	///<Release a new key from the active list of key pressed

	#ifdef UGKOS_WINDOWS
	inline void SetActivetokens(const WPARAM c){ SetActivetokens(Tokenize(c)); };	///<Set a new key into the active list of key pressed
	inline void ResetActivetokens(const WPARAM c){ ResetActivetokens(Tokenize(c)); };///<Release a new key from the active list of key pressed
	#endif // UGKOS_WINDOWS
public:
	...
	void SetKeyDelay(float *, CK_TOKEN); //Set especific delay to one key
