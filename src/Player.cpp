...
void CPlayer::ManageEvent	(unsigned int Event)
{
		switch (Event)
		{
...
void CPlayer::UserShoot	()
{
	...
	if (Alive != true) return;	///No gun is fired when the player is dead
		...
		ShootsManager.NewShoot(Player[CurrentPlayer].ShootType, P, S);				//Auxiliar shoots with Bonus Weapon
		if(Player[CurrentPlayer].ShootType == CSH_PLAYER3D_CHEVRON){
			P.v[XDIM] += 0.5;
			ShootsManager.nextShootVisible = true;
			ShootsManager.NewShoot(Player[CurrentPlayer].ShootType, P, S);
			P.v[XDIM] -= 0.5;
			P.v[XDIM] -= 0.5;
			ShootsManager.nextShootVisible = true;
			ShootsManager.NewShoot(Player[CurrentPlayer].ShootType, P, S);
		...
		if (Player[CurrentPlayer].Laser[CP_LEFT_LASER].Alive)						//Left Laser shoot
		{
		...
		if (Player[CurrentPlayer].Laser[CP_RIGHT_LASER].Alive)						//Right Laser shoot
		{
			P.v[XDIM]		= Position.v[XDIM] + .9f;
			P.v[YDIM]		= Position.v[YDIM] + .8f;
			P.v[ZDIM]		= Mesh->modelo.pos.z;
...
void CPlayer::DiscreteUserShoot	()
{
	Vector	P,	//Shoot initial Position
			S;	//Shoot initial Speed
	...
	if (Alive != true) return;	///No gun is fired when the player is dead
		...
		DiscreteShootsManager.DiscreteNewShoot(Player[CurrentPlayer].ShootType, P, S);
		if(Player[CurrentPlayer].ShootType == CSH_PLAYER3D_CHEVRON){
			P.v[XDIM] += 0.5;
		...
		if (Player[CurrentPlayer].Laser[CP_LEFT_LASER].Alive)						// le laser de gauche tire
		{
			P.v[XDIM]		= Position.v[XDIM] - .9f;
			P.v[YDIM]		= Position.v[YDIM] + .8f;
			P.v[ZDIM]		= Mesh->modelo.pos.z;
		...
		if (Player[CurrentPlayer].Laser[CP_RIGHT_LASER].Alive)						// le laser de droite tire
		{
			P.v[XDIM]		= Position.v[XDIM] + .9f;
			P.v[YDIM]		= Position.v[YDIM] + .8f;
			P.v[ZDIM]		= Mesh->modelo.pos.z;
