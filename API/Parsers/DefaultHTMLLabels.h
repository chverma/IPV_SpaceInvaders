...
typedef enum {
	...
	TITLE_D,		//HTML tag
	TYPE_D,
	UNKNOWN_D,		//This tag is for management purpouses only. There is no correspondence to any string label in the HTML file
	VERSION_D,
	MAXTAGS_D		//This tag is for management purpouses only. There is no correspondence to any string label in the HTML file
} DHTML_TagType;

typedef char DHTML_TagsArray[MAXTAGS_D][DHTML_TAG_MAX_LONG];
